...
- **Never use a partial endpoint URL as an `<a href>` target in any HTMX response fragment.** Partial endpoints return fragment HTML only — no `<html>`, no base layout, no CSS variables resolved. Clicking an `<a href="/partial/endpoint">` in an HTMX response triggers full-page navigation to a bare, unstyled fragment. Any inline success/error message that links elsewhere must link to a full-page route (e.g. `/discovered`, not `/discovered/{id}/detail`). Check every route that returns `RedirectResponse` — each is a latent picture-in-picture bug waiting to be triggered via HTMX. This also breaks debugging/testing, not just production links: navigating directly to a fragment endpoint URL (e.g. to inspect or manually drive a form during QA or browser automation) loads the fragment with no `htmx.js` from the page shell. Any `<form hx-post="...">` inside it silently degrades to a native browser form submit — which defaults to GET, dumping the entire payload into the URL query string with no error surfaced. To test or drive an htmx-wired form, always go through the real parent page and trigger the element that loads it via htmx (e.g. `document.querySelector('[data-id="N"]').click()`), never load the fragment URL standalone.
